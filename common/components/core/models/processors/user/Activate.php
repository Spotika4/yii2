...
class Activate extends \common\components\core\models\base\UpdateProcessor {
	...
	public function query(){
		return \common\components\core\models\ar\User::find()->from('{{%user}} u')->select(['u.*', 'o.value'])
			...
			->where(['o.key' => 'verification_token', 'o.value' => $this->token]);
	}
	...
	public function beforeSave(){
		...
		if($this->object->save(false)){
			$this->object->deleteOption('verification_token');
			$this->addMessage(\Yii::t('yiicore/user', 'user_activation_success'));
			return true;
		}
		return false;
	}
